...
public class Math3d
{
    ...
    public class BezierCurve
    {
        ...
        public Vector3 point(float t)
        {
            ...
            {
                for (int j = 0; j < allPoints[i+1].Length; ++j)
                {
                    allPoints[i + 1][j] = Vector3.Lerp(allPoints[i][j], allPoints[i][j + 1], t);
                }
            }

            return allPoints[allPoints.Length - 1][0];
        }

        public BezierCurve inverse()
        {
            Vector3[] invArray = (Vector3[]) allPoints[0].Clone();
            for (int i = 0; i < invArray.Length; ++i)
            {
                invArray[i] = allPoints[0][allPoints[0].Length - 1 - i];
            }
            return new BezierCurve(invArray);
        }
    }
}